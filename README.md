...
16 scenario playbooks registered as real Skills under `.claude/skills/`, and a
reference library of 14 tool categories and 270+ individual tool references
...
└── .claude/
    ...
    └── reference/
        ...
        └── <category>/        # one directory per tool category, not a Skill
            ├── INDEX.md        # Golden Path + Decision Tree for that category
...
| `reporting` | Testing complete, need to produce a report |
...
Each category's `INDEX.md` leads with a Golden Path (default tool per scenario)
...
## Adding a new tool, category, or scenario

1. New tool in an existing category: add `.claude/reference/<category>/tools/<name>.md`
   ...
   References), then link it from that category's `INDEX.md`.
2. New category: add `.claude/reference/<category>/INDEX.md` (Golden Path +
   Decision Tree) and a `tools/` subdirectory, then add it to the Tool
   Categories table in `AGENTS.md` and to this README.
3. New scenario: add `.claude/skills/<name>/SKILL.md` with `name`/`description`
   frontmatter, and register it in `.claude/reference/playbook-selection.md`'s
   decision tree and cross-reference map.
